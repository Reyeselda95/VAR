...
class Wander {
	...
	void commandCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
		...
		int midValue=ceil(totalValues/2);

		bool right=false,left=false,mid=false;

		// 0.1 grados un valor
		//
		for(int i=0; i< 1000;++i){
			if(msg->ranges[midValue+i]<=1){
				right=true;
			}
	...
	// Bucle principal
	void bucle() {
		ros::Rate rate(1000); // Especifica el tiempo de bucle en Hertzios. Ahora está en ciclo por segundo, pero normalmente usaremos un valor de 10 (un ciclo cada 100ms).
